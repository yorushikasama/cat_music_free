...
class PluginManager implements IPluginManager, IInjectable {
    ...
    private nameIndex = new Map<string, Plugin>();
    private hashIndex = new Map<string, Plugin>();
    ...
    private setPlugins(plugins: Plugin[]) {
        ...
        for (const plugin of plugins) {
            ...
            this.hashIndex.set(plugin.hash, plugin);
        }

        // 清理缓存中已卸载的插件
        const cachedKeys = pluginCacheStore.getAllKeys();
        cachedKeys.forEach(key => {
            if (!this.nameIndex.has(key) && !this.hashIndex.has(key)) {
    ...
    async setup() {
        try {
            ...
            const lazyLoadEnabled = this.appConfigService.getConfig(
                "basic.lazyLoadPlugin",
            );

            // 1. 并行读取所有插件文件内容
            const pluginEntries = await Promise.all(
                pluginsFileItems.map(async pluginFileItem => {
                    if (
                    ...
                    if (
                        ...
                        if (!lazyProps || typeof lazyProps !== "object") {
                ...
                for (let i = 0; i < allPlugins.length; ++i) {
                    ...
                    if (plugin.state === PluginState.Initializing) {
                        try {
                            ...
                            errorLog("懒加载插件失败", { name: plugin.name, error: err?.message ?? err });
                        }
                    }
                }
            });
